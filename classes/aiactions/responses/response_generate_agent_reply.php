...
/**
 * Final agent reply response.
 *
 * @package    aiprovider_wunderbyte
 * @copyright  2026 Wunderbyte GmbH
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class response_generate_agent_reply extends \core_ai\aiactions\responses\response_base {
    /** @var string|null Provider response id. */
    private ?string $id = null;

    /** @var string|null Provider-side response fingerprint. */
    private ?string $fingerprint = null;

    /** @var string|null Generated user-facing content. */
    private ?string $generatedcontent = null;

    /** @var string|null Generation finish reason. */
    private ?string $finishreason = null;

    /** @var int|null Prompt token usage. */
    private ?int $prompttokens = null;

    /** @var int|null Completion token usage. */
    ...
    public function __construct(bool $success, int $errorcode = 0, string $error = '', string $errormessage = '') {
        parent::__construct(
            success: $success,
            actionname: 'generate_agent_reply',
            errorcode: $errorcode,
            error: $error,
